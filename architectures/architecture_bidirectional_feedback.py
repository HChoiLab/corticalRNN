...
class microcircuit(nn.Module):
    def __init__(self, seq_len=1, n_features=64, hidden_dim=8, pop_list = [2,5,4], pEdge=0.3, bsize=100, device=device, manual_seed=0, task='recon', n_classes=None, latticeDim=2):
        ...
        self.WFB = nn.Linear(in_features=(pop_list[0])*hidden_dim, out_features=(pop_list[1])*hidden_dim) ##higher 4 --> lower 2
        ...
        self.BBFB = torch.ones((pop_list[0])*hidden_dim,(pop_list[1])*hidden_dim).T.bernoulli_(p=pEdge,generator=torch.manual_seed(manual_seed)).to(device)
        ...
        self.projFB = torch.rand(1,seq_len,pop_list[1]*hidden_dim)
        self.projFB = self.projFB.repeat(self.bsize,1,1).to(device)
    ...
    def forward(self, x):
        ...
        opProjFB = torch.zeros(nSamp,nSteps,self.pop[1]*self.hidden_dim).to(self.device)
        ...
        tempFB = torch.rand(nSamp,self.seq_len,self.pop[1]*self.hidden_dim).to(self.device)
        # tempFBa = torch.rand(nSamp,self.seq_len,self.pop[1]*self.hidden_dim).to(self.device)
        # tempFBb = torch.rand(nSamp,self.seq_len,self.pop[2]*self.hidden_dim).to(self.device)
        ...
        for ii in range(nSteps):
            ...
            concatL23 = torch.cat((projL423,self.projFB),dim=2)
            # concatL23 = projL423
            ...
            diffL23[:,ii,:] = opL23[:,ii,:] - torch.squeeze(self.projFB) ## Difference between o/p of lower areal RNN and higher areal feedback
            ...
            self.WFB.weight.data.mul_(self.BBFB) ## Mask the weights
            projFB = self.WFB(xH4)
            projFB = self.relu(projFB) ##Only excitatory signals propagate inter-areally
            opProjFB[:,ii,:] = torch.squeeze(projFB)

            # self.WFBa.weight.data.mul_(self.BBFBa) ## Mask the weights
            # projFBa = self.WFBa(xH56)
            # projFBa = self.relu(projFBa) ##Only excitatory signals propagate inter-areally
            # opProjFBa[:,ii,:] = torch.squeeze(projFBa)
            #
            # self.WFBb.weight.data.mul_(self.BBFBb) ## Mask the weights
            # projFBb = self.WFBb(xH56)
            # projFBb = self.relu(projFBb) ##Only excitatory signals propagate inter-areally
            # opProjFBb[:,ii,:] = torch.squeeze(projFBb)
        ...
        return pred, (opL23,opL4,opL56,opH23,opH4,opH56), (diffL23, diffL56), (opProjFF,opProjFB)
